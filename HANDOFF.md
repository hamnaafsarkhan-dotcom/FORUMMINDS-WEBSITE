...
- **Exception: `register.html`'s form can only be submitted on the deployed site** — see §5.1c. It does a normal HTML POST that only Netlify catches, so submitting from `file://` or a local static server just navigates to `thank-you.html` (or nowhere) with nothing recorded. Everything else on the page — the dropdown, the validation, the error summary — works locally.
...
1. **~~No form endpoint~~ — RESOLVED 2026-08-05, registration re-pointed at Netlify Forms 2026-08-17.**
   ...
   **How the wiring works:**
   ...
   - `register.html`'s `<form>` carries `name="registration"`, `method="POST"`, `action="thank-you.html"`, `data-netlify="true"` and `netlify-honeypot="website"`, plus a hidden `form-name` input repeating the name. Netlify reads the form and its field list **from the published HTML at deploy time**.
   - `assets/js/register.js` has `var FORM_NAME = "registration"` where `ENDPOINT` used to be. **It does not intercept the submit.** The handler validates and calls `preventDefault()` only when a field is wrong (or in manual mode); a valid form does a **plain HTML POST**, which is what Netlify catches natively.
   - **`thank-you.html`** is the confirmation — a normal page in the site's shell, `noindex` and not in `sitemap.xml`, reachable only by Netlify redirecting there after it stores the submission. It carries no recap of the delegate's answers: a native POST navigates away and takes the values with it, and the page cannot read them back, so it confirms only what is certainly true.
   - **Do not disable the submit button before the form data set is built.** `register.js` defers it to a `setTimeout(…, 0)`. Disabled controls are excluded from the submission; this button has no `name` so it contributes nothing either way, but the ordering matters for any control that does.
   ...
   **An earlier cut of this posted by `fetch` to `/`** so an in-page confirmation panel could be shown. That is a supported Netlify pattern, but it puts a moving part between the delegate and a recorded booking, and its failure branch is worded identically to the old PHP one — which made a form-detection problem look like a leftover PHP call. The fetch, `confirmSuccess()` and the `#form-success` panel were all removed on 2026-08-17; the history of that day has them if the pattern is ever wanted back.

   **What to check after the first deploy:**

   1. **The form is registered.** Netlify dashboard → **Forms** should list `registration` after a deploy. **This is the first thing to check when submissions fail** — and note that Netlify no longer enables form detection automatically on new sites: it has to be switched on under **Site configuration → Forms**, and the site redeployed *after* switching it on, before `data-netlify="true"` does anything at all. Until then every submission 404s.
